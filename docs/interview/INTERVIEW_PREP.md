...
**关键点：** 每个包的 4 字节长度头写入 `header_cache_` 向量，而不是每个包的 data() 前面。这是因为 Packet 的 data() 指向的是包体[msg_id+body]，封装层的长度由 OutboxBatcher 在发送时才添加——这是一种解耦设计。
...
- `Actor::enqueue_message()` 使用 `IntrusiveList` 的 lock-free push（基于 CAS）
...
**执行路径：**
1. `handler()` 在调用者线程上**同步启动**（`initial_suspend = suspend_never`）
...
5. `final_suspend` → 如果被另一个协程 `co_await` 等待，恢复等待者；如果是 `DetachedTask`，什么都不做

**注意：** C++20 协程是**栈无关**的（stackless），每个协程对象 = 一次堆分配（promise_type + 参数捕获）。AegisEngine 的 Task 只有 `co_await` 时才有堆分配，同步路径（如果 child task 已就绪）不走分配——称为「零开销等待」。
...
> AegisEngine 是一个基于 C++20 协程 + io_uring 的 MMO 游戏服务器引擎。它使用 Actor 模型组织业务逻辑——每个玩家、场景、NPC 都是一个 Actor，有自己的消息队列和 worker 线程绑定。IO 层完全异步，通过 io_uring 提交/完成队列实现零系统调用开销。网络协议使用 protobuf 序列化，加上一个轻量的大端序帧头。对象池贯穿整个项目——Packet、Actor、TimerNode 都从池中分配。状态同步方面，使用基于 AOI 网格的脏标记驱动方案，批量打包、共享 buffer、零拷贝广播。
