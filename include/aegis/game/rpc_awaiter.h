...
namespace aegis::core
{
    ...
    public:
        ...
        std::coroutine_handle<> consume(RpcId id, void *result_ptr)
        {
            ...
            if (it->second.timer_id != 0 && t_current_worker)
                t_current_worker->time_wheel().cancel_timer(it->second.timer_id);
        ...
        RpcId next_id()
        {
            ...
            return (static_cast<uint64_t>(Worker::get_current_id()) << WORKER_SHIFT) | id;
        }
    ...
    public:
        ...
        RpcAwaiter(ActorID target, ActorMessage *msg,
        ...
        bool await_suspend(std::coroutine_handle<> handle)
        {
            ...
            if (t_current_worker)
            {
                timer_id = t_current_worker->time_wheel().add_timer(
                    timeout_ms_, [id]()
            ...
            auto *target = ActorRegistry::instance().get(target_id_);
            if (!target)
            {
                timed_out_ = true;
                return false;
                              ...
                              uint32_t timeout_ms = kRpcDefaultTimeoutMs)
    {
        ...
        return RpcAwaiter<ResT>(target, static_cast<ActorMessage *>(msg),
